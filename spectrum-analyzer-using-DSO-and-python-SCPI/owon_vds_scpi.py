...
*** VERSIONE 46 (Correzione Parametro *ADC?) ***
- get_adc_data: Modificato per inviare il *numero* del canale (es. "1")
  invece della stringa (es. "CH1"), basandosi sull'ambiguità
  del manuale.
"""
...
# --- Sottosistemi :TRIGGER (invariati) ---
...
# --- Sottosistemi Principali (invariati) ---
...
class OwonVDS_SCPI:
    ...
    def _query_command(self, command, is_binary=False):
        ...
        try:
            ...
            self.disconnect() # OK disconnettere su TIMEOUT
            return None
        except Exception as e:
            print(f"Errore durante il comando query '{command.strip()}': {e}")
            # NON DISCONNETTERE per un errore generico (es. utf-8 decode)
            return None
    ...
    # --- FUNZIONI AGGIUNTE (V43 - Robuste) ---
    def set_run(self):
        """
        Avvia l'acquisizione (RUN) in modo robusto.
        Usa *RUNStop che è un toggle e controlla la risposta.
        """
        print("  set_run(): Invio *RUNStop per controllare/impostare lo stato...")
        response = self._query_command("*RUNStop") 
        
        # Rende il controllo case-insensitive e ignora spazi
        if "STOP" in response.upper().strip(): 
            # Lo scope era in RUN, il comando lo ha messo in STOP.
            # Dobbiamo inviare di nuovo il comando per rimetterlo in RUN.
            print("  set_run(): Stato RUN->STOP. Rinvio *RUNStop per tornare a RUN...")
            response = self._query_command("*RUNStop")
        
        print(f"  set_run(): Stato finale: {response}")


    def set_stop(self):
        """
        Ferma l'acquisizione (STOP) in modo robusto.
        Usa *RUNStop che è un toggle e controlla la risposta.
        """
        print("  set_stop(): Invio *RUNStop per controllare/impostare lo stato...")
        response = self._query_command("*RUNStop")
        
        # Rende il controllo case-insensitive e ignora spazi
        if "RUN" in response.upper().strip(): 
            # Lo scope era in STOP, il comando lo ha messo in RUN.
            # Dobbiamo inviare di nuovo il comando per rimetterlo in STOP.
            print("  set_stop(): Stato STOP->RUN. Rinvio *RUNStop per tornare a STOP...")
            response = self._query_command("*RUNStop")
        
        print(f"  set_stop(): Stato finale: {response}")
    # --- FINE MODIFICA ---
        
    
    # --- *** INIZIO MODIFICA CHIAVE *** ---
    def get_adc_data(self, channel_num=1):
        # Il manuale è ambiguo:
        # La tabella dice {CH1|CH2...}
        # La spiegazione dice {1|2...}
        # Abbiamo provato "CH1" e ha fallito (errore 4 byte).
        # Ora proviamo con il numero.
        channel_str = str(channel_num)
        
        print(f"  DEBUG: Invio comando '*ADC? {channel_str}'") # Aggiunto debug
        
        # 1. Chiedi i dati come BINARI
        raw_data = self._query_command(f"*ADC? {channel_str}", is_binary=True) 
        
        if not raw_data:
            print(f"Errore: Nessun dato ADC (binario) ricevuto per {channel_str}")
            return None
            
        try:
            # 2. Controlla che NON sia un messaggio di errore di testo (es. 4 byte)
            #    I dati validi devono essere almeno 500 byte.
            #    La tua immagine mostra "D 5k", quindi ci aspettiamo
            #    MOLTI più dati di 500. Ma 500 è il minimo per il nostro calcolo FFT.
            if len(raw_data) < 500:
                print(f"Errore: Dati ADC incompleti. Ricevuti {len(raw_data)} byte, attesi almeno 500.")
                try:
                    print(f"  -> Possibile messaggio di errore: {raw_data.decode('utf-8').strip()}")
                except:
                    pass # Non era un errore di testo
                return None
            
            # 3. Converte i dati BINARI in un array di numeri
            #    Prendiamo solo i primi 500 punti, che corrispondono
            #    all'aspettativa della libreria originale.
